...
export const uploadAudioFile = async (audioBlob: Blob, fileName: string): Promise<string> => {
  try {
    ...
    const { error } = await supabase.storage.from("voice-recordings").upload(fileName, audioBlob, {
...
export const deleteFile = async (fileName: string): Promise<void> => {
  try {
    const { error } = await supabase.storage.from("voice-recordings").remove([fileName]);
